(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.viewerjs) {
      window.wwload.viewerjs = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/viewerjs/dist/viewer.min", "css!libs/viewerjs/dist/viewer.min"], function (Viewer) {
        window.wwload.viewerjs = true;
        window.Viewer = Viewer;
        replace();
        fncallback();
      });
    } else if (window.wwload.viewerjs === "wait") {
      setTimeout(function () {
  ...
  function viewer_btn($ele) {
    ...
    var options = {
      ...
      view: function (e) {
        //console.log(e.type);
        $.wwclass.helper.anijsTrigger($ele, "viewsuccess");
      },
      viewed: function (e) {
        //console.log(e.type);
        $.wwclass.helper.anijsTrigger($ele, "viewedsuccess");
      },
      zoom: function (e) {
        //console.log(e.type);
      },
  ...
  function processElement($ele) {
    ...
    var viewerbtn = $ele.attr("data--viewerbtn");
